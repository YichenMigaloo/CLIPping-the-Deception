...
def get_5_keypoint(shape):
    ...
    reye = np.array(center(get_point(45), get_point(42)),
                    dtype=np.int).reshape(-1, 2)
    nose = np.array(get_point(30), dtype=np.int).reshape(-1, 2)
...
def get_mask(shape, img, std=20, deform=True, restrict_mask=None):
    mask_type = ['normal-hull', 'inner-hull',
                 'inner-hull-no-eyebrow', 'rect', 'mouth-hull', 'whole-hull']
    max_mask = get_hull_mask(img, shape, 'normal-hull')
    if deform:
